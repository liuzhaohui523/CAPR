...
class MixDataset2Cam(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if dataset_idx > 0 and dataset_idx < 3:
            ...
            target['uvd_heatmap'] =torch.zeros((29, 64, 64, 64))
            #POTTER的数据指标
            # target['initial_view_B'] =torch.zeros((64,64,64))
            # target['initial_view_R'] =torch.zeros((64,64,64))
            # target['initial_view_L'] =torch.zeros((64,64,64))
            #feater的数据指标
            target['initial_view_B'] =torch.zeros((32,64,64))
            target['initial_view_R'] =torch.zeros((32,64,64))
            target['initial_view_L'] =torch.zeros((32,64,64))
        else:
            assert set(target.keys()).issubset(self.data_domain), (set(target.keys()) - self.data_domain, self.data_domain - set(target.keys()),)
            if 'uvd_heatmap' not in target:
                target['uvd_heatmap'] =torch.zeros((29, 64, 64, 64))
            #POTTER的数据指标
            # if 'initial_view_B' not in target:
            #     target['initial_view_B'] =torch.zeros((64,64,64))
            # if 'initial_view_R' not in target:
            #     target['initial_view_R'] =torch.zeros((64,64,64))
            # if 'initial_view_L' not in target:
            #     target['initial_view_L'] =torch.zeros((64,64,64))
