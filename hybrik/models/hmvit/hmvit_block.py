...
class Attention(nn.Module):
    def __init__(self, C, H, W, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,chunk_size=64):
        super().__init__()
    ...
    def split_chunks(self, x, chunk_size):
        ...
        进行分块注意力的先前准备
        """
        return [x[:, :, :, i:i + chunk_size] for i in range(0, x.size(-1), chunk_size)]
    def forward(self, x):
        B, C, H, W = x.shape
        x_H = x.transpose(2, 3)
        ...
        #使用分块注意力来计算注意力
        attn_W_chunks = []
        for q_W_chunk, k_W_chunk, v_W_chunk in zip(self.split_chunks(q_W, self.chunk_size),
                                                   self.split_chunks(k_W, self.chunk_size),
                                                   self.split_chunks(v_W, self.chunk_size)):
            attn_W_chunk = (q_W_chunk.transpose(-3, -2) @ torch.permute(k_W_chunk, (0, 1, 3, 4, 2))) * self.scale_W
            attn_W_chunk = attn_W_chunk.softmax(dim=-1)
            attn_W_chunk = self.attn_drop_W(attn_W_chunk)
            attn_W_chunks.append(attn_W_chunk @ v_W_chunk.transpose(2, 3))

        x_W = torch.cat(attn_W_chunks, dim=-1).transpose(1, 3).reshape(B, C, H, W)
        ...
        #分块注意力
        attn_H_chunks = []
        for q_H_chunk, k_H_chunk, v_H_chunk in zip(self.split_chunks(q_H, self.chunk_size),
                                                   self.split_chunks(k_H, self.chunk_size),
                                                   self.split_chunks(v_H, self.chunk_size)):
            attn_H_chunk = (q_H_chunk.transpose(-3, -2) @ torch.permute(k_H_chunk, (0, 1, 3, 4, 2))) * self.scale_H
            attn_H_chunk = attn_H_chunk.softmax(dim=-1)
            attn_H_chunk = self.attn_drop_H(attn_H_chunk)
            attn_H_chunks.append(attn_H_chunk @ v_H_chunk.transpose(2, 3))

        x_H = torch.cat(attn_H_chunks, dim=-1).transpose(1, 3).reshape(B, C, W, H)
...
if __name__ == '__main__':
    ...
    model = HMVIT_block(C=32, H=64, W=64, depth=8, ratio=4,
    ...
    print(f"当前 GPU 内存占用: {current_memory / 1024 ** 2:.2f} MB")
    print(f"最大 GPU 内存占用: {max_memory / 1024 ** 2:.2f} MB")